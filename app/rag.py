...
_AGG_RE = re.compile(
    # Раньше здесь были ещё и голые \bвсе\b|\bвсех\b|\bвсём\b|\bвсей\b — самые частые
    # слова в русском («спасибо всем», «у всех клиентов», «расскажи обо всём»), они
    # ложно уводили обычные вопросы в агрегационный режим (в т.ч. в медленный full_scan,
    # который может отвечать минуты). Специфичные фразы («список всех» и т.п.) уже ловят
    # реальный сценарий отдельно, без голых слов сигнал не теряется.
    r"перечисли|перечислите|весь список|полный список|список всех|список всей|"
    r"какие (есть|бывают|существуют)|"
