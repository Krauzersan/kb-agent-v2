...
_DECOMPOSE_SYSTEM = (
    ...
    "«нюансы»). Если вопрос на самом деле простой и раскладывать нечего — верни массив "
    "из одного этого же вопроса.\n\n"
...
_PLAN_SYSTEM = (
    ...
    "заголовок и одну фразу, что именно в нём раскрыть.\n\n"
...
def _decompose(question: str) -> list[str]:
    try:
        raw = llm.complete(_DECOMPOSE_SYSTEM, question, max_tokens=400)
...
def _plan(question: str, facts_digest: str) -> list[dict]:
    ...
    try:
        raw = llm.complete(_PLAN_SYSTEM, user, max_tokens=600)
        items = _parse_json(raw)
    except Exception:
