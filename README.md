...
> [!TIP]
> DEEP can genuinely take several minutes end to end (a dozen sequential model calls). Messaging channels are unaffected — their webhooks reply 200 immediately and deliver the answer as a follow-up message — but the admin panel's Test Agent page waits on one synchronous HTTP request. If you're running behind a reverse proxy with a finite response timeout (nginx's `proxy_read_timeout` defaults to 60s), raise it for this path — Caddy (the bundled `docker-compose.yml` setup) has no such default and doesn't need this.
...
> [!TIP]
> DEEP реально может занимать несколько минут целиком (десяток последовательных вызовов модели). На мессенджеры это не влияет — их вебхуки отвечают 200 сразу, а ответ приходит отдельным сообщением, — но страница «Тест агента» в админке ждёт один синхронный HTTP-запрос. Если сервис стоит за реверс-прокси с конечным таймаутом ответа (у nginx `proxy_read_timeout` по умолчанию 60с) — увеличьте его для этого пути. У Caddy (из комплектного `docker-compose.yml`) такого таймаута по умолчанию нет, там это не нужно.
